...
// 3. Send two analog data points read through the ADC to Azure IoT Hub.
...
// Function to generate ADC data/telemetry
static void SendADCdata(void);
// The size of ADC sample in bits
static int sampleBitCount = -1;
// The maximum voltage for ADC calculations
...
static ExitCode InitPeripheralsAndHandlers(void)
{
    ...
    //Open ADC and get Fd, Sample Bit Count (Should be 12) and set reference voltage
...
// <summary>
///    Reads the ADC channel output for sensor data and sends to IoT Hub.
/// </summary>
void SendADCdata(void)
{
    ...
    float voltage = ((float)value * sampleMaxVoltage) / (float)((1 << sampleBitCount) - 1) * 10.0;

    Log_Debug("Sensor1 value is %.3f ma\n", voltage);

    char tempBuffer[20];
    int len = snprintf(tempBuffer, 20, "%2.3f", voltage);
    if (len > 0)
        SendTelemetry("Sensor1ma", tempBuffer);

    /* char *pjsonBuffer = (char *)malloc(15);
    if (pjsonBuffer == NULL) {
        Log_Debug("ERROR: not enough memory to send telemetry");
    }

    snprintf(pjsonBuffer,  15, "{\"MA\":\"%.3f\"}", voltage);
    Log_Debug("\n[Info] Sending telemetry %s\n", pjsonBuffer);
    AzureIoT_SendMessage(pjsonBuffer);
    free(pjsonBuffer);*/

    uint32_t value2;
    int result2 = ADC_Poll(adcControllerFd, 1, &value2);
    if (result2 < -1) {
        Log_Debug("ADC_Poll failed with error: %s (%d)\n", strerror(errno), errno);
     //   terminationRequired = true;
        return;
    ...
    float voltage2 = ((float)value2 * sampleMaxVoltage) / (float)((1 << sampleBitCount) - 1) * 10.0;

    Log_Debug("Sensor2 value is %.3f ma\n", voltage2);

    char tempBuffer2[20];
    int len2 = snprintf(tempBuffer2, 20, "%2.3f", voltage2);
    if (len2 > 0)
        SendTelemetry("Sensor2ma", tempBuffer2);
